...
#define PRODUCT_ID 0x0009 //PIC18F4550
...
static int pic18f_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	printk("%x", endpoint->bEndpointAddress);
	printk("%x", endpoint->bmAttributes);
...
static int __init pic18f_init(void)
{
    ...
    result =  usb_register(&pic18f_driver);
    return result;
}
...
MODULE_DESCRIPTION("USB kernel driver to interface PIC18F4550 microcontroller");
MODULE_LICENSE("GPL");
